...
export class SearchComponent implements OnInit, OnDestroy {
  ...
  filteredItems = computed(() => {
    ...
    // Si pas de tri spécial nécessaire (tri par date décroissant par défaut côté Firestore), retourner les items directement
    if (f.sortBy === 'date' && f.sortOrder === 'desc') {
      return items;
    }

    // Sinon, créer une copie et trier
    let result = [...items];
  ...
  toggleAdvancedFilters() {
    console.log('🔄 toggleAdvancedFilters called, current value:', this.showAdvancedFilters());
    this.showAdvancedFilters.set(!this.showAdvancedFilters());
    console.log('🔄 showAdvancedFilters set to:', this.showAdvancedFilters());
  }
  ...
  updateFilter<K extends keyof AdvancedSearchFilters>(key: K, value: AdvancedSearchFilters[K]) {
    console.log('🔄 updateFilter called:', key, '=', value, 'type:', typeof value);
    ...
    if (this.firestoreFilters.includes(key)) {
      console.log('🔄 Reloading data for filter:', key);
      this.loadInitialData();
      this.loadTotalCount();
    }
  }
  ...
  // Ajoutez ces méthodes dans la classe SearchComponent

  // Méthode pour obtenir la valeur d'un select
  getSelectValue(event: Event): string {
    const select = event.target as HTMLSelectElement;
    return select.value;
  }

  // Méthode pour obtenir la valeur d'un input
  getInputValue(event: Event): string {
    const input = event.target as HTMLInputElement;
    return input.value;
  }

  // Helper fortement typé pour les <select> afin d'éviter les erreurs de type dans les templates
  updateFilterFromSelect<K extends keyof AdvancedSearchFilters>(key: K, event: Event) {
    const select = event.target as HTMLSelectElement;
    const value = select.value as AdvancedSearchFilters[K];
    this.updateFilter(key, value);
  }

  // Helper fortement typé pour les <input> (text)
  updateFilterFromInput<K extends keyof AdvancedSearchFilters>(key: K, event: Event) {
    const input = event.target as HTMLInputElement;
    const value = input.value as AdvancedSearchFilters[K];
    this.updateFilter(key, value);
  }

  // Méthode pour gérer les changements de date (déjà existante)
  onDateChange(field: 'dateFrom' | 'dateTo', event: Event) {
    const input = event.target as HTMLInputElement;
    const value = input.value;
    
    if (value) {
      // Créer une date à partir de la valeur YYYY-MM-DD
      const date = new Date(value + 'T00:00:00');
      this.updateFilter(field, date);
    } else {
      this.updateFilter(field, null);
    }
  }
