...
export class MapComponent implements OnInit, OnDestroy, OnChanges {
   ...
   @Input() latitude = 48.8626;
   @Input() longitude = 2.3344;
   @Input() zoomLevel = 13;
   @Input() markers: MapMarker[] = [];
   @Input() defaultRouteOrigin: { lat: number; lng: number } | null = {
      lat: 48.8626,
      lng: 2.3344,
   }; // Place de l'Échelle
   @Input() defaultRouteDestination: { lat: number; lng: number } | null = {
      lat: 48.8809,
      lng: 2.3553,
   }; // Gare du Nord
   @Input() showDefaultRoute = true;
   ...
   private loadGoogleMapsScript() {
      ...
      if (!apiKey) {
         console.error(
            '⚠️ Google Maps API Key est invalide! Veuillez configurer votre clé API.',
         );
      ...
      script.onerror = () => {
         console.error(
            "Erreur: Google Maps API n'a pas pu être chargée. Vérifiez votre clé API.",
         );
   ...
   private async initializeMap() {
      setTimeout(async () => {
         try {
            ...
            if (
               this.showDefaultRoute &&
               this.defaultRouteOrigin &&
               this.defaultRouteDestination
            ) {
   ...
   showRoute(
      ...
      this.directionsService.route(request, (result: any, status: any) => {
         if (
            ...
            this.alternativeRoutes = result.routes
               .map((route: any) => {
               ...
               .filter(
                  (route: any) => route.distance !== 'N/A' && route.duration !== 'N/A',
               );
            ...
            if (this.alternativeRoutes.length > 0) {
               ...
               console.log(
                  `Route principale: ${firstRoute.distance}, Durée: ${firstRoute.duration}`,
               );
            }
         } else {
   ...
   drawPath(points: { lat: number; lng: number }[], color = '#0000FF', weight = 2) {
      if (points.length < 2) {
   ...
   selectRoute(index: number) {
      ...
      const request = {
         ...
         travelMode:
            window.google.maps.TravelMode[this.selectedTravelMode] ||
