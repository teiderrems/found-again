  ...
  get email() {
   ...
   async confirmEmail(){
      ...
      if (this.confirmEmailForm.valid) {
         this.authService.resetPassword(this.confirmEmailForm.get('email')?.value as string)
            .then(()=>this.router.navigateByUrl('/login'))
            .catch(error=> { console.error(error); this.errorMessage ="Identifiants Incorrects.";});
      }
   }
}
